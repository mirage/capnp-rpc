...
(* This is the raw, untyped interface. The application does not use this directly. *)
class type value =
  (* A value is used to access a response struct, a field within a response, or a capability.
     This may involve sending messages over the network. *)
  object
    method apply : Xform.t list -> value
    (* Get a transformed version of this value. *)

    method call :
      Xform.t list ->             (* Transforms to apply on target *)
      req_msg -> value Ro_array.t ->   (* Data and caps for request *)
      value * (resp_msg * value Ro_array.t) Lwt.t
    (* Invoke this value. Only makes sense for capabilities.
       The result is a value (which can be pipelined with other calls) and a (message, caps)
       pair (which can't). *)
  end
